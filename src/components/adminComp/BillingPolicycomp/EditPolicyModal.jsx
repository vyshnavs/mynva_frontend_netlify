...
export default function EditPolicyModal({ policy, close, refresh }) {
  ...
  useEffect(() => {
    if (policy) {
      setForm({
        ...
        fixedChargePerCycle: policy.fixedChargePerCycle,
        fuelSurchargePerUnit: policy.fuelSurchargePerUnit,
        electricityDutyPercent: policy.electricityDutyPercent,
        effectiveFrom: policy.effectiveFrom?.split("T")[0],
        status: policy.status
      });
      setSlabs(policy.slabs || []);
    }
  }, [policy]);
  ...
  const handleSlabChange = (i, field, val) => {
    const updated = [...slabs];
    updated[i][field] = val === "" ? "" : field === "rate" ? parseFloat(val) || 0 : parseInt(val) || 0;
    setSlabs(updated);
  };
  ...
  const handleSubmit = async () => {
    try {
      await api.put(`/admin/billing/${policy._id}`, { ...form, slabs });
      refresh();
      close();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`${bgClass} rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto`}>
        <div className="p-6">
          <h3 className="text-2xl font-bold mb-4">Edit Billing Policy</h3>
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  type="number"
                  value={form.cycleDays || 60}
                  onChange={(e) => setForm({ ...form, cycleDays: parseInt(e.target.value) })}
                  className={`w-full px-3 py-2 border rounded-lg ${inputClass}`}
                />
              </div>
              <div>
                ...
                <input
                  type="number"
                  step="0.01"
                  value={form.fixedChargePerCycle || 0}
                  onChange={(e) => setForm({ ...form, fixedChargePerCycle: parseFloat(e.target.value) || 0 })}
                  className={`w-full px-3 py-2 border rounded-lg ${inputClass}`}
                />
              </div>
              <div>
                ...
                <input
                  type="number"
                  step="0.01"
                  value={form.fuelSurchargePerUnit || 0}
                  onChange={(e) => setForm({ ...form, fuelSurchargePerUnit: parseFloat(e.target.value) || 0 })}
                  className={`w-full px-3 py-2 border rounded-lg ${inputClass}`}
                />
              </div>
              <div>
                ...
                <input
                  type="number"
                  step="0.01"
                  value={form.electricityDutyPercent || 0}
                  onChange={(e) => setForm({ ...form, electricityDutyPercent: parseFloat(e.target.value) || 0 })}
                  className={`w-full px-3 py-2 border rounded-lg ${inputClass}`}
                />
              </div>
              <div>
